...
- **Three attention regimes** — full-context, sliding-window (GPT-OSS, Gemma, Mistral v0.1), and linear/recurrent (Kimi K3's KDA, Qwen3-Next), the last holding a *constant* state that never grows with context. GPT-OSS-120B at 128K halves, 5.4 → 2.7 GiB; Kimi K3 at 1M drops 3.7×, 31.4 → 8.5 GiB. Catalog geometry is taken from each model's published `config.json`.
