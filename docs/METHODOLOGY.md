...
**Across nodes the numbers are withheld, not estimated.** A replica wider than a node all-reduces over a fabric this plan knows nothing about, so throughput **and TTFT** are reported as `—` until the per-GPU fabric bandwidth is supplied. Memory sizing and GPU count are unaffected.

TTFT belongs in that list, which is not obvious: prefill runs the *same* two all-reduces per layer, but over the whole prompt instead of a decode batch. For GLM-5.2 at TP16 and 1M context that is **2.26 TB** of collective traffic against **28.8 MB** per decode step — 90 s at a 50 GB/s fabric, larger than the entire 64 s TTFT it was previously left out of. Prefill is bandwidth-bound on the interconnect where decode is latency-bound on it, which is why this half is computable exactly and the decode half is only a floor. A modern Blackwell node budgets roughly 100 GB/s per GPU, where the collective costs about 6% of the step for a 78-layer MLA model at TP16 — and about 20% at 25 GB/s. Quoting a figure that silently assumed the collective were free would be the most confident and least supportable number on the page.
