    ...
    <section>
      {#if R}
        ...
        {#if R.throughput_suppressed}
          <div class="state warn"><b>Throughput and TTFT withheld.</b> {R.throughput_suppressed}<div class="sfind">Memory sizing and GPU count are unaffected. <b>TTFT is withheld too</b>: prefill all-reduces on every layer across the same fabric, over the whole prompt rather than a decode batch — for a 1M-context TP16 plan that is 2.26 TB against 0.03 GB per decode step, so it can exceed the entire compute time it was being left out of.</div></div>
        {/if}
        ...
        <div class="kpis">
          ...
          <div class="kpi t"><div class="v">{R.ttft_suppressed ? '—' : ttftLabel(R.ttft_ms)}</div><div class="l">Time to first token</div><div class="cav">{R.ttft_suppressed ? 'withheld · fabric unknown' : `±50% · ${R.ttft_compute_bound ? 'compute-bound' : 'bandwidth floor'}`}</div></div>
