...
- **Mixed-precision checkpoints** — frontier low-bit releases rarely quantise everything (GLM-5.2 NVFP4: *"only MoE expert linears"*; DeepSeek-V4: *"MoE experts FP4, remaining FP8"*). Models can declare a dense remainder and the precision it keeps, instead of one bytes/param for the whole file.
- **Prefill activations** — the runtime reserve scales with `--max-num-batched-tokens × hidden_size`, not a flat constant. Llama-3.3-70B needs 2.5 GiB at vLLM's default chunk and 7.5 GiB at a 64K chunk — memory that is no longer KV cache.
