    ...
    <section>
      {#if R}
        ...
        {#if R.throughput_suppressed}
          <div class="state warn"><b>Throughput and TTFT withheld.</b> {R.throughput_suppressed}<div class="sfind">Memory sizing, GPU count and the KV plan are unaffected — only the compute-derived figures are withheld, because they describe a deployment other than the one planned.</div></div>
        {/if}
          ...
          <table class="rubric"><thead><tr><th class="num">Concurrency</th><th class="num">GPUs</th><th class="num">Pods</th><th class="num">TTFT</th><th class="num">tok/s · req</th><th class="num">tok/s · total</th><th></th></tr></thead><tbody>
            {#each sweep as s}
              <tr class:cur={s.concurrency === conc} class:infeasible={!s.feasible}>
                ...
                {#if s.feasible}
                  <td class="num">{s.gpus}{#if s.tight}<span class="badge tightb" title="under 10% pod headroom">tight</span>{/if}</td><td class="num">{s.pods} × TP{s.tp}</td><td class="num">{s.ttft_suppressed ? '—' : `~${ttftLabel(s.ttft_ms)}`}</td><td class="num">{s.throughput_suppressed ? '—' : `~${s.decode_tps_per_request}`}</td><td class="num">{s.throughput_suppressed ? '—' : `~${s.throughput_tokens_per_sec.toLocaleString()}`}</td>
...
{:else if tab === 'methodology'}
  <section class="panel doc">
    ...
    <p><code>FFN_width</code> is the width one token traverses in one layer — <code>intermediate_size</code> for a dense model; for MoE, <code>moe_intermediate_size × num_experts_per_tok</code> <b>plus the always-on shared experts</b>, since a token materialises activations in every routed expert it visits and in every shared one. Shared experts are no rounding error: DeepSeek-V3's one against top-8 is an eighth of the total. That makes it <code>2048 × (8+1) = 18,432</code> per token — exactly the <code>intermediate_size</code> its dense layers use.</p>
    <p>Every seeded model declares its width, read from its published <code>config.json</code>; the ratios span 2.6× hidden (DeepSeek-V3) to 7.7× (Kimi K3, 16 routed + 2 shared experts of 3072), so no single ratio would have served. Undeclared, the engine assumes <b>3.5 × hidden_size</b> — the SwiGLU convention, which over-reserves for MoE and is the safe direction.</p>
    ...
    <p class="note"><b>Across nodes the numbers are withheld, not estimated.</b> A replica wider than a node all-reduces over a fabric this plan knows nothing about, so throughput <b>and TTFT</b> read <code>—</code> until the per-GPU fabric bandwidth is supplied. Memory sizing and GPU count are unaffected. At roughly 100 GB/s per GPU the decode collective costs about 6% of the step for a 78-layer MLA model at TP16, and about 20% at 25 GB/s.</p>
    <p>TTFT belongs in that list, which is not obvious: prefill runs the <em>same</em> two all-reduces per layer, but over the whole prompt instead of a decode batch. For GLM-5.2 at TP16 and 1M context that is <b>2.26 TB</b> of collective traffic against <b>28.8 MB</b> per decode step — 90 s at a 50 GB/s fabric, larger than the entire 64 s TTFT it was previously being left out of. Prefill is bandwidth-bound on the interconnect where decode is latency-bound on it, which is why this half is computable exactly and the decode half is only a floor.</p>
    ...
    <p><b>A fallback kernel withholds the performance figures.</b> The roofline assumes native kernels — <code>bandwidth × MBU</code> for decode, <code>FLOPS × MFU</code> for prefill. On a weight-only path neither holds: activations stay 16-bit and the low-precision tensor cores are never reached, so a throughput or TTFT number would describe a deployment other than the one planned. Those read <code>—</code>; memory fit, GPU count and the KV plan are unchanged. That is 98 of the catalogue's 628 feasible combinations — FP8 on Ampere, INT4 on AMD, MXFP4 before Hopper, NVFP4 before Blackwell. <b>GGUF is the deliberate exception:</b> its degradation is a different <em>runtime</em> rather than a fallback kernel, and llama.cpp decode is still bandwidth-bound, so withholding those figures would suppress them for the wrong reason.</p>
